...
if __name__ == '__main__':
    mem = CoronelMemory("auditoria-hotel-123")
    mem.store_plan(["fase1", "fase2"])
    task_id = mem.add_task({"type": "inspeccion", "area": "piso1"})
    mem.update_task_status(task_id, TaskStatus.DISPATCHED)
    mem.update_task_status(task_id, TaskStatus.COMPLETED, {"resultado": "OK"})

    print("\n--- Reporte de Memoria ---")
    print(mem.get_full_report())
    print("\n--- Log de Ejecución ---")
    for entry in mem.execution_log:
        print(entry)
