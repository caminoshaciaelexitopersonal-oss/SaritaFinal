...
from apps.operational_intelligence.models import SaaSMetric, ChurnRiskScore, RevenueForecast, UnitEconomics, OperationalRiskIndex
...
def run_certification():
    ...
    now = timezone.now()
    for day in range(14):
        # We create metrics directly to control the timestamp
        SaaSMetric.objects.create(
            metric_name='USAGE_UNITS',
            value=Decimal('100') if day < 7 else Decimal('20'),
            timestamp=now - timezone.timedelta(days=14 - day),
            meta_data={'customer_id': str(customer_id), 'feature': 'AI_TOKENS'}
        )
    ...
    assert metrics['mrr'] == Decimal('200.00'), f"MRR Incorrecto: {metrics['mrr']}"
