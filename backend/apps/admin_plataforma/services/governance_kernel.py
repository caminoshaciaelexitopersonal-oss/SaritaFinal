...
class GovernanceKernel:
    ...
    def _dispatch(self, intention: GovernanceIntention, parameters: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if intention.domain == "operativo_artesano":
            ...
            if details.get("status") == "FAILED":
                raise ValueError(details.get("error", "Error en la ejecución del agente de taller."))
...
# Dominio: Artesanos (Fase 15)
GovernanceKernel.register_intention(GovernanceIntention(
    name="REGISTER_RAW_MATERIAL",
    domain="operativo_artesano",
