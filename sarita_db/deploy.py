...
def deploy():
    ...
    print("--- Iniciando Despliegue GESTIÓN ARCHIVÍSTICA INSTITUCIONAL (Fase 10.7) ---")
    ...
    print(f"Registrando versión v10.7_institutional_archival (Checksum: {final_checksum[:10]})")
    subprocess.run(['psql', db_url, '-c', f"SELECT core.apply_schema_version('v10.7_institutional_archival', '{final_checksum}')"], check=True)

    print("--- DESPLIEGUE ARCHIVÍSTICO COMPLETADO ---")
