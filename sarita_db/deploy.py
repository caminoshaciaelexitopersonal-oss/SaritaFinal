...
def deploy():
    ...
    print("--- Iniciando Despliegue BANCO CENTRAL (Fase 10 Hardening Final) ---")
    ...
    # Registrar versión final
    final_checksum = global_checksum.hexdigest()
    print(f"Registrando versión v10_hardening (Checksum: {final_checksum[:10]})")
    subprocess.run(['psql', db_url, '-c', f"SELECT core.apply_schema_version('v10_hardening', '{final_checksum}')"], check=True)

    print("--- DESPLIEGUE FINALIZADO EXITOSAMENTE: NIVEL BANCO CENTRAL ---")
