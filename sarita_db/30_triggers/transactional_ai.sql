...
CREATE OR REPLACE FUNCTION ai_memory.fn_monitor_fraud_and_abandonment()
RETURNS TRIGGER AS $$
BEGIN
    ...
    ELSIF TG_TABLE_NAME = 'payment_attempts' AND NEW.status = 'FAILED' THEN
        INSERT INTO ai_memory.agent_events (source_table, record_id, event_payload, tenant_id)
        VALUES ('payments.payment_attempts', NEW.id, jsonb_build_object('warning', 'potential_fraud'), NEW.tenant_id);
    END IF;
...
DROP TRIGGER IF EXISTS trg_ai_fraud ON payments.payment_attempts;
CREATE TRIGGER trg_ai_fraud AFTER INSERT ON payments.payment_attempts FOR EACH ROW EXECUTE FUNCTION ai_memory.fn_monitor_fraud_and_abandonment();
