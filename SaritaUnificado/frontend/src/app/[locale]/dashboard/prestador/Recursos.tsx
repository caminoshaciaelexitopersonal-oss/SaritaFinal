...
const Recursos = () => {
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm('¿Estás seguro de que quieres eliminar este recurso?')) {
      try {
        await api.delete(`/empresa/recursos/${id}/`);
        toast.success('Recurso eliminado con éxito');
        fetchRecursos();
      } catch (err: any) {
        toast.error('Error al eliminar el recurso: ' + (err.response?.data?.detail || err.message));
      }
    }
  };
            ...
            {recursos.map((recurso) => (
              <tr key={recurso.id}>
                ...
                <td className="py-2 px-4 border-b">
                  <button onClick={() => openModalForEdit(recurso)} className="text-blue-500 hover:underline mr-2">Editar</button>
                  <button onClick={() => handleDelete(recurso.id)} className="text-red-500 hover:underline">Eliminar</button>
                </td>
              </tr>
            ))}
          </tbody>
