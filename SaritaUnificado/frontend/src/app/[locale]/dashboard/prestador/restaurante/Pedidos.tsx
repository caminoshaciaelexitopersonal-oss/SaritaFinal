...
const Pedidos = () => {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      setProductosMenu(productosRes.data.results || productosRes.data);
    } catch (err: any) {
      setError(
      ...
      >
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div>
            <label>Mesa</label>
            <select
              {...register('mesa', { valueAsNumber: true })}
          ...
          {fields.map((field, index) => (
            <div key={field.id} className="flex items-center space-x-2">
              <select
                {...register(`items.${index}.producto`, { valueAsNumber: true })}
          ...
          <button
            type="button"
            onClick={() => append({ producto: 0, cantidad: 1 })}
            ...
            >
              <div className="flex justify-between items-center">
                <h2 className="font-bold">
                  Pedido #{pedido.id} - Mesa {pedido.mesa?.numero_mesa || 'N/A'}
                </h2>
                <span
