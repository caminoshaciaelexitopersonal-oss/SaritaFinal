...
const ReglasPrecio = () => {
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm('¿Estás seguro de que quieres eliminar esta regla de precio?')) {
      try {
        await api.delete(`/empresa/reglas-precio/${id}/`);
        toast.success('Regla de precio eliminada con éxito');
        fetchReglasYProductos();
      } catch (err: any) {
        toast.error('Error al eliminar la regla de precio: ' + (err.response?.data?.detail || err.message));
      }
    }
  };
