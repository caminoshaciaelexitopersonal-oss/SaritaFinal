...
const Clientes = () => {
  ...
  const fetchClientes = useCallback(async () => {
    try {
      setIsLoading(true);
      const response = await api.get('/v1/mi-negocio/clientes/');
  ...
  const onSubmit: SubmitHandler<FormInputs> = async (data) => {
    try {
      const url = editingCliente
        ? `/v1/mi-negocio/clientes/${editingCliente.id}/`
        : '/v1/mi-negocio/clientes/';
            ...
            {clientes.map((cliente) => (
              <tr key={cliente.id}>
                ...
                <td className="py-2 px-4 border-b">
                  <button onClick={() => openModalForEdit(cliente)} className="text-blue-500 hover:underline">Editar</button>
                </td>
              </tr>
            ))}
          </tbody>
