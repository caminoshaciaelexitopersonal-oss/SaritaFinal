...
const GaleriaManager = () => {
  ...
  const fetchImagenes = async () => {
    try {
      ...
      setImagenes(response.data.galeria_imagenes || []);
    } catch (error) {
  ...
  const onSubmit: SubmitHandler<FormData> = async (data) => {
    ...
    // El backend asocia el prestador automáticamente a través del usuario autenticado
    ...
    try {
      ...
      reset();
      fetchImagenes(); // Recargar la lista de imágenes
    } catch (error) {
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm('¿Estás seguro de que deseas eliminar esta imagen?')) {
      try {
        ...
        fetchImagenes(); // Recargar la lista
      } catch (error) {
