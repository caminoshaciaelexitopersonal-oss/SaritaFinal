...
const ValoracionesManager = () => {
  ...
  const fetchResenas = useCallback(async () => {
    try {
      const response = await request('/valoraciones/');
      setResenas(response.results || response);
    } catch (error) {
  ...
  const handleEnviarRespuesta = async (id: number) => {
    ...
    try {
      await request(`/valoraciones/${id}/`, {
        method: 'PATCH',
        body: JSON.stringify({ respuesta_prestador: textoRespuesta }),
      });
  ...
  if (apiLoading && resenas.length === 0) {
