...
async def route_llm_request(
    ...
    En modo de prueba, devuelve una respuesta JSON simulada.
    """
    # --- MODO DE PRUEBA SIMULADO ---
    if os.environ.get("TEST_MODE") == "True":
        print("[LLM Router] MODO DE PRUEBA ACTIVO. Devolviendo respuesta simulada.")
        if "NationalPlan" in prompt:
            return json.dumps({
                "plan": [{
                    "directive_description": "Busca los 3 principales atractivos turísticos de Puerto Gaitán y crea una publicación en el blog sobre ellos.",
                    "responsible_department": "Meta"
                }]
            })
        elif "DepartmentalPlan" in prompt:
            return json.dumps({
                "plan": [
                    {"task_description": "Buscar los 3 principales atractivos turísticos de Puerto Gaitán.", "responsible_colonel": "TurismoCoronel"},
                    {"task_description": "Crear una nueva publicación de blog con la información de los atractivos encontrados.", "responsible_colonel": "TurismoCoronel"}
                ]
            })
        elif "TacticalPlan" in prompt:
             return json.dumps({
                "plan": [
                    {"task_description": "Buscar los 3 principales atractivos turísticos de Puerto Gaitán.", "responsible_captain": "Atractivos"},
                    {"task_description": "Crear una nueva publicación de blog con la información de los atractivos encontrados.", "responsible_captain": "Publicaciones"}
                ]
            })
        else: # Fallback para otros agentes (Capitanes, etc.)
             return json.dumps({"final_report": "Tarea simulada completada con éxito por el agente de bajo nivel."})
