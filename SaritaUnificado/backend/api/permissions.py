...
class IsPrestadorOwner(BasePermission):
    """
    Permiso para verificar que el usuario es un prestador y es el dueño del objeto.
    """
    def has_permission(self, request, view):
        return bool(
            request.user and
            request.user.is_authenticated and
            request.user.role == CustomUser.Role.PRESTADOR and
            hasattr(request.user, 'perfil_prestador')
        )

    def has_object_permission(self, request, view, obj):
        # El perfil_prestador del usuario debe ser el mismo que el prestador del objeto
