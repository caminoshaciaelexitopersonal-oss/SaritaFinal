...
class UserManagementAPITests(APITestCase):
    ...
    def test_admin_can_update_any_user(self):
        ...
        self.client.credentials(**self._get_auth_header(self.admin_token))
        response = self.client.patch(self.turista_detail_url, data)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_admin_can_delete_any_user(self):
        ...
        self.client.credentials(**self._get_auth_header(self.admin_token))
        response = self.client.delete(self.turista_detail_url)
    ...
    def test_funcionario_can_create_allowed_roles(self):
        ...
        self.client.credentials(**self._get_auth_header(self.funcionario_token))
        response = self.client.post(self.list_url, data_turista)
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
    ...
    def test_funcionario_cannot_create_disallowed_roles(self):
        ...
        self.client.credentials(**self._get_auth_header(self.funcionario_token))
        response = self.client.post(self.list_url, data_admin)
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)

        data_func = {'username': 'intentofunc', 'password': 'password123', 'role': 'FUNCIONARIO_DIRECTIVO'}
        self.client.credentials(**self._get_auth_header(self.funcionario_token))
        response = self.client.post(self.list_url, data_func)
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
    ...
    def test_funcionario_cannot_update_disallowed_user(self):
        ...
        self.client.credentials(**self._get_auth_header(self.funcionario_token))
        response = self.client.patch(self.funcionario_detail_url, data)
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
    ...
    def test_funcionario_cannot_delete_disallowed_user(self):
        ...
        self.client.credentials(**self._get_auth_header(self.funcionario_token))
        response = self.client.delete(self.funcionario_detail_url)
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
